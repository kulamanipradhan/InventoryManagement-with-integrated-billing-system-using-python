...
class supplierClass:
    def __init__(self,root):
        ...
        #=== searchFrame=====
        #options
        lbl_serach =Label(self.root,text="Search by Invoice No",bg="white",font=("goudy old styl",15))
        lbl_serach.place(x=700,y=80)
        
        #text serach
        txt_search =Entry(self.root,textvariable=self.var_searchtxt,font=("goudy old style",15),bg="lightyellow").place(x=770,y=80)
        btn_search =Button(self.root,text="Search",command=self.search,font=("goudy old style",15),bg="green",fg="white",cursor = "hand2").place(x=990,y=80,width=100,height=28)
        #title
        ...
        #button
        btn_add =Button(self.root,text="Save",command=self.add,font=("goudy old style",15),bg="blue",fg="white",cursor = "hand2").place(x=180,y=320,width=110,height=28)
        btn_update =Button(self.root,text="Update",command=self.update,font=("goudy old style",15),bg="green",fg="white",cursor = "hand2").place(x=300,y=320,width=110,height=28)
        btn_delete =Button(self.root,text="Delete",command=self.delete,font=("goudy old style",15),bg="orange",fg="white",cursor = "hand2").place(x=420,y=320,width=110,height=28)
        btn_clear =Button(self.root,text="Clear",command=self.clear,font=("goudy old style",15),bg="brown",fg="white",cursor = "hand2").place(x=540,y=320,width=110,height=28)
        #employee details
        ...
        self.supplierTable=ttk.Treeview(sup_frame,column=("invoice","name","contact","desc"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        ...
        self.supplierTable.heading("desc",text="Descriptin")
    ...
    def add(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","invoice number already assigned,try diffrerent",parent=self.root)
                else:
